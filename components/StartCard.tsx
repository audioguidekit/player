...
import { ArrowUpToLine, Clock3, MapPin, Headphones, Download, CheckCircle2 } from 'lucide-react';
    ...
    <div className="px-8 pt-8 flex flex-col items-center text-center w-full" style={{ paddingBottom: 'calc(2.5rem + env(safe-area-inset-bottom, 0px))' }}>
      ...
      <button
        onClick={(e) => {
          ...
          console.log('🔘 Button clicked', {
            offlineAvailable: tour.offlineAvailable,
            isDownloaded,
            isDownloading,
            hasOnDownload: !!onDownload
          });

          // If tour requires offline download and isn't downloaded yet, start download
          if (tour.offlineAvailable && !isDownloaded && !isDownloading && onDownload) {
            console.log('▶️ Calling onDownload');
            onDownload();
          } else if (!isDownloading && (!tour.offlineAvailable || isDownloaded)) {
            // Only allow tour start if not downloading AND (offline not required OR already downloaded)
            console.log('▶️ Calling onAction');
            onAction();
          }
        }}
      ...
      >
        ...
        <span className="relative z-10 flex items-center gap-3">
          {isDownloading ? (
            <>
              <Download size={20} strokeWidth={2.5} className="animate-pulse" />
          ...
          {isDownloaded && tour.offlineAvailable && !isDownloading && (
            <CheckCircle2 size={18} strokeWidth={2.5} className="text-green-400" />
          )}
        </span>
      </button>
    </div>
