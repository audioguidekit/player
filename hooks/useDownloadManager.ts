 ...
 */
export const useDownloadManager = (tour: TourData | null): UseDownloadManagerReturn => {
  ...
  const startDownload = useCallback(async () => {
    ...
    if (!tour || isDownloading || isDownloaded) {
      console.log('🛑 Early return from startDownload', {
        noTour: !tour,
        alreadyDownloading: isDownloading,
    ...
    try {
      ...
      console.log(`Tour ${tour.id} downloaded successfully. Total size: ${(totalSize / 1024 / 1024).toFixed(2)} MB`);
    } catch (err) {
  ...
  }, [tour, isDownloading, isDownloaded, extractAssetUrls]);
